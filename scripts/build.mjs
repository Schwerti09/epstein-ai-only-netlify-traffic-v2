...
async function generate(){
  ...
  for (const p of must){
    if (!fs.existsSync(path.join(root, p))) throw new Error("Missing core file: " + p);
  }
  ...
  // sitemap.xml (relative URLs are fine for Netlify; you can switch to absolute later with SITE_URL)
...
async function main(){
  if (CHECK_ONLY){
    const must = [
      "site/index.html","site/styles.css","site/js/app.js","site/assets/favicon.svg",
      "netlify/functions/health.mjs","netlify/functions/ai-search.mjs","netlify/functions/checkout.mjs",
      "netlify/functions/verify.mjs","netlify/functions/whoami.mjs","netlify/functions/og.mjs",
      "data/queries.json"
    ];
    const missing = must.filter(p => !fs.existsSync(path.join(root, p)));
    if (missing.length){
